...
/**
 * Serviço responsável por salvar {@link PessoaJuridica} e garantir que ela tenha um usuário
 * de acesso criado no sistema (quando necessário).
 *
 * <p>Essa classe fica na camada de serviço, ou seja, ela concentra regras de negócio.
 * O controller normalmente chama este serviço para executar a lógica completa de cadastro.</p>
 *
 * <p>Em termos simples: quando uma empresa (PessoaJuridica) é cadastrada, este serviço:
 * salva a empresa + ajusta os endereços + cria o usuário de login (se ainda não existir)
 * + define senha criptografada + adiciona o acesso padrão + envia e-mail com login e senha.</p>
 */
@Service
public class PessoaUserService {

    /**
     * Repositório responsável pelas operações de banco relacionadas a {@link Usuario}.
     */
    @Autowired
    private UsuarioRepository usuarioRepository;

    /**
     * Repositório responsável pelas operações de banco relacionadas a {@link PessoaJuridica}.
     */
    @Autowired
    private PessoaRepository pessoaRepository;

    /**
     * Serviço responsável por enviar e-mail.
     * Aqui é usado para enviar o login e a senha gerados para a empresa cadastrada.
     */
    @Autowired
    ...
    /**
     * Salva uma {@link PessoaJuridica} no banco e cria um usuário de acesso caso ainda não exista.
     *
     * <p>Esse método faz mais do que apenas “salvar a empresa”. Ele garante também que:</p>
     * <p>
    ...
    public PessoaJuridica salvarPessoaJuridica(PessoaJuridica pessoaJuridica) {
        ...
        Usuario usuarioPJ =
                usuarioRepository.findUserByPessoa(pessoaJuridica.getId(), pessoaJuridica.getEmail());
        ...
        if (usuarioPJ == null) {
            ...
            // Monta um HTML simples com os dados de acesso
     ...
     *
     * Primeiro ele garante que os endereços estão corretos:
     * se você não setar o “pessoa” e o “empresa” dentro do endereço,
     * o Hibernate pode tentar salvar o endereço sem vínculo e dar erro,
     * ou salvar de um jeito incompleto.
     *
     * Depois ele salva a PessoaJuridica e pega o ID gerado.
     * Esse ID é importante porque, sem ele, você não consegue relacionar
     * corretamente o usuário com a empresa.
     *
     * Aí vem a parte mais importante: criar o usuário de acesso.
     * O método procura se já existe um usuário para essa pessoa e e-mail.
     * Se não existir, cria:
     * - login = e-mail da empresa
     * - senha temporária gerada na hora
     * - senha criptografada com BCrypt (isso é o que faz a senha ser segura)
     * - salva no banco
     * - coloca o acesso padrão (role) para esse usuário
     *
     * Por fim, ele envia um e-mail com login e senha.
     * Repare que o sistema guarda a senha criptografada,
     * mas manda para o usuário a senha original (não criptografada),
     * porque só assim o usuário consegue fazer o primeiro login.
     *
     * Em resumo:
     * esse método salva a empresa e “prepara o acesso dela ao sistema”
